...
// test all variations.
func makeProvers(trie *Trie) []func(key []byte) *db.MemoryDB {
	var provers []func(key []byte) *db.MemoryDB

	// Create a direct trie based Merkle prover
	provers = append(provers, func(key []byte) *db.MemoryDB {
	...
	provers = append(provers, func(key []byte) *db.MemoryDB {
		...
		if it := NewIterator(trie.NodeIterator(key)); it.Next() && bytes.Equal(key, it.Key) {
			for _, p := range it.Prove() {
				proof.Put(common.Keccak256(p), p)
			}
		}
		return proof
	})
...
func TestBadProof(t *testing.T) {
	...
	for i, prover := range makeProvers(trie) {
		for _, kv := range vals {
			...
			mutateByte(val)
			proof.Put(common.Keccak256(val), val)
