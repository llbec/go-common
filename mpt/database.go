...
type Database struct {
	diskdb db.DB // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb db.DB) *Database {
...
func NewDatabaseWithCache(diskdb db.DB, cache int) *Database {
...
func (db *Database) DiskDB() db.KeyValueReader {
	return db.diskdb
}
...
func (db *Database) Dereference(root common.Hash) {
	...
	if root == (common.Hash{}) {
		log.Printf("Attempted to dereference the trie cache meta root")
		return
	}
	db.lock.Lock()
	defer db.lock.Unlock()
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	if flushPreimages {
		for hash, preimage := range db.preimages {
			if err := batch.Put(db.secureKey(hash[:]), preimage); err != nil {
				log.Printf("Failed to commit preimage from trie database, err: %v", err)
				return err
			}
	...
	for size > limit && oldest != (common.Hash{}) {
		...
		if batch.ValueSize() >= IdealBatchSize {
			if err := batch.Write(); err != nil {
				log.Printf("Failed to write flush list to disk, err: %v", err)
				return err
			}
			batch.Reset()
		}
...
func (db *Database) Commit(node common.Hash, report bool) error {
	...
	if err := db.commit(node, batch, uncacher); err != nil {
		log.Printf("Failed to commit trie from trie database, err:%v", err)
		return err
	}
	// Trie mostly committed to disk, flush any batch leftovers
	if err := batch.Write(); err != nil {
		log.Printf("Failed to write trie to disk, err:%v", err)
		return err
	}
...
func (db *Database) commit(hash common.Hash, batch db.Batch, uncacher *cleaner) error {
